import 'package:dr_oplawrence_bible/data/sources/remote/bible_api_services.dart';
...
// ============== bible chapter ==============
final bibleChapterProvider = FutureProvider.family<List<BibleChapter>, String>((
  ref,
  bookId,
) async {
  final repository = ref.read(bibleRepositoryProvider);
  ...
  return (response as List).map((json) => BibleChapter.fromJson(json)).toList();
});

final selectedChapterIndexProvider = StateProvider<int?>((ref) => null);
final bookChapterCountProvider = FutureProvider.family<int, String>((
  ref,
  bookId,
) async {
...
final completedChaptersProvider = StateProvider<Map<String, Set<String>>>(
  (ref) => {},
);
// ============== bible verse ==============
final bibleVerseProvider = FutureProvider.family<List<BibleVerse>, String>((
  ref,
  chapterId,
) async {
  final repository = ref.read(bibleRepositoryProvider);
  ...
  return (response as List).map((json) => BibleVerse.fromJson(json)).toList();
});
